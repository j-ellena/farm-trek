...
const visible = 'translate(0, 0)'
const hiddenAbove = 'translate(25px, -100px)'
const hiddenBelow = 'translate(-25px, 100px)'

const useToggleStyle = makeStyles((theme: Theme) => createStyles({
    toggleButton: {
    ...
    darkIcon: {
        color: yellow[800],
        transition,
        transform: theme.palette.type === dark ? visible : hiddenAbove
    },
    lightIcon: {
        color: yellow[500],
        transition,
        transform: theme.palette.type === dark ? hiddenBelow : visible
    }
}))
